...
test.describe('PTO Request Workflow', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for the page to be ready (may be landing page, login, or dashboard)
  ...
  test('manager can approve PTO request', async ({ page }) => {
    ...
    const firstRequest = page
      .locator('[data-testid="pto-request-item"]')
      .first();
  ...
  test('manager can deny PTO request with reason', async ({ page }) => {
    ...
    await page.fill(
      '[name="denialReason"]',
      'Insufficient coverage during that period'
    );
  ...
  test('system prevents overlapping PTO requests', async ({ page }) => {
    ...
    // Navigate to PTO request page with better wait condition
    const requestTimeOffButton = page.locator('text=Request Time Off');
    const isVisible = await requestTimeOffButton
  ...
  test('PTO request workflow respects permissions', async ({
    ...
    const hasPermissionError =
      (await page
        .locator('text=/permission|unauthorized|access denied/i')
        .count()) > 0;
