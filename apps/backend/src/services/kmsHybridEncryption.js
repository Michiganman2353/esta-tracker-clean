 ...
 * ```
 */
export async function encryptWithKMS(data, keyVersion) {
    try {
 ...
 * ```
 */
export async function decryptWithKMS(payload) {
    try {
        ...
        const aesKey = await kmsService.asymmetricDecrypt(payload.encryptedAESKey, payload.keyVersion);
 ...
 * @returns KMS hybrid encryption result
 */
export async function encryptFileWithKMS(data, keyVersion) {
 ...
 */
export async function decryptFileWithKMS(payload) {
    try {
        const encryptedData = Buffer.from(payload.encryptedData, 'base64');
        const iv = Buffer.from(payload.iv, 'base64');
        const authTag = Buffer.from(payload.authTag, 'base64');
 ...
 */
export async function getKMSPublicKey(keyVersion) {
    const keyInfo = await kmsService.getPublicKey(keyVersion);
 ...
 */
export async function isKMSAvailable() {
