...
describe('Edge Hybrid Encryption Module', () => {
  ...
  describe('generateEdgeRSAKeys', () => {
    ...
    it('should generate unique key pairs', async () => {
      ...
      expect(JSON.stringify(exported1.publicKey)).not.toBe(
        JSON.stringify(exported2.publicKey)
      );
      expect(JSON.stringify(exported1.privateKey)).not.toBe(
        JSON.stringify(exported2.privateKey)
      );
    });
    ...
    }, 15000); // Increase timeout for 4096-bit key generation
  });
  ...
  describe('edgeEncryptBinaryData / edgeDecryptBinaryData', () => {
    it('should encrypt and decrypt binary data', async () => {
      ...
      const encrypted = await edgeEncryptBinaryData(
        originalData,
        testKeyPair.publicKey
      );
